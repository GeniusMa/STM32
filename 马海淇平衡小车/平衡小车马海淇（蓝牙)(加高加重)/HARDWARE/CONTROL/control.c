...
*/
float Med_Angle=-9;		//机械中值。---在这里修改你的机械中值即可。
float Target_Speed=0;	//期望速度（俯仰）。---二次开发接口，用于控制小车前进后退及其速度。
float Turn_Speed=0;		//期望速度（偏航）

float 
	Vertical_Kp=-340,//直立环KP、KD
	Vertical_Kd=-1.6;
float 
	Velocity_Kp=0.40,//速度环KP、KI
	Velocity_Ki=0.0020;
float 
	Turn_Kd=-0.6,//转向环KP、KD
	Turn_Kp=-20;

#define SPEED_Y 22 //俯仰(前后)最大设定速度
#define SPEED_Z 75//偏航(左右)最大设定速度 
